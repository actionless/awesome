...
local function apply_shape(self)
    local s = self._tooltip._private.shape
    ...
    s(cr, w, h, unpack(self._tooltip._private.shape_args or {}))
    cr:fill()
    ...
    local bw = (self._tooltip._private.border_width
    ...
    local bc = self._tooltip._private.border_color
        or beautiful.tooltip_border_color
        or beautiful.border_normal
        or "#ffcb60"

    cr:translate(bw, bw)
    s(cr, w-2*bw, h-2*bw, unpack(self._tooltip._private.shape_args or {}))
    cr:set_line_width(bw)
    cr:set_source(color(bc))
    cr:stroke_preserve()
    cr:clip()

    local bg = self._tooltip._private.bg
...
local function apply_outside_mode(self)
    ...
    if position ~= self._private.direction then
        -- Re-apply the shape.
        apply_shape(self.wibox)
    end
