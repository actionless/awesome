...
function ratio:layout(context, width, height)
    ...
    for k, v in ipairs(self._private.widgets) do
        ...
        if has_stragety then
            ...
            is_void = (v.visible == false) or (dir == "x" and fit_w == 0) or (dir == "y" and fit_h == 0)
    ...
    for _, entry in ipairs(preliminary_results) do
        ...
        if strategy ~= "default" then
            if dir == "y" then
                h = is_void and 0 or h + (to_redistribute / (#preliminary_results - void_count))
                y = space_front + real_pos
                real_pos = real_pos + h

            else
                w = is_void and 0 or w + (to_redistribute / (#preliminary_results - void_count))
